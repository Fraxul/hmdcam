...
struct RHIBlendStateDescriptorElement {
  RHIBlendStateDescriptorElement() :
    blendEnabled(false),
    colorSource(kBlendOne),
    colorDest(kBlendZero),
    alphaSource(kBlendOne),
    alphaDest(kBlendZero),
    colorFunc(kBlendFuncAdd),
    alphaFunc(kBlendFuncAdd) {}
  RHIBlendStateDescriptorElement(RHIBlendWeight source, RHIBlendWeight dest, RHIBlendFunc func = kBlendFuncAdd) :
    blendEnabled(true),
    colorSource(source),
    colorDest(dest),
    alphaSource(source),
    ...
    colorFunc(func),
    alphaFunc(func) {}
  RHIBlendStateDescriptorElement(RHIBlendWeight colorSource_, RHIBlendWeight colorDest_, RHIBlendWeight alphaSource_, RHIBlendWeight alphaDest_, RHIBlendFunc func = kBlendFuncAdd) :
    blendEnabled(true),
    colorSource(colorSource_),
    colorDest(colorDest_),
    alphaSource(alphaSource_),
    alphaDest(alphaDest_),
    colorFunc(func),
    alphaFunc(func) {}
  bool blendEnabled;
...
struct RHIBlendStateDescriptor {
  RHIBlendStateDescriptor(const RHIBlendStateDescriptorElement& el, const glm::vec4& constantColor_ = glm::vec4(0.0f)) :
    constantColor(constantColor_) { targetBlendStates.push_back(el); }
  RHIBlendStateDescriptor(const std::initializer_list<RHIBlendStateDescriptorElement>& els, const glm::vec4& constantColor_ = glm::vec4(0.0f)) :
    targetBlendStates(els.begin(), els.end()),
    constantColor(constantColor_) {}
  RHIBlendStateDescriptor() :
    constantColor(glm::vec4(0.0f)) {}
