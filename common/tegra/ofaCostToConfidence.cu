...
__global__ void ofaCostToConfidenceKernel(CUtexObject ofaCost, int lowCostThreshold, int highCostThreshold, PtrStepSz<uint8_t> confidenceMat) {
...
void ofaCostToConfidence(CUtexObject ofaCost, cv::cuda::GpuMat& confidenceMat, uint8_t lowCostThreshold, uint8_t highCostThreshold, CUstream stream) {
  ...
  ofaCostToConfidenceKernel<<<grid, block, 0, stream>>>(ofaCost, lowCostThreshold, highCostThreshold,
